...
impl LogTailTool {
    pub fn new(session: Arc<Session>) -> Self {
        Self {
            session,
            descriptor: ToolDescriptor::agent("log_tail", schema_for::<LogTailArgs>())
                .with_description("Tail a configured SSH log source."),
        }
    }
}
...
impl Tool for LogTailTool {
    ...
    async fn invoke(&self, ctx: &ToolCtx<'_>) -> Result<ToolResponse, ToolError> {
        invoke_session_tool(&self.session, "log_tail", ctx).await
    }
}
