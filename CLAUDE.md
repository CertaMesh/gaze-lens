...
v0.5.4 shipped 2026-08-25 (see CHANGELOG.md). It adds CLI-only opt-in sanitized source diagnostics, shared `value` where-clause aliases, explicit 1000-row cap enforcement, and more reliable SSH tunnel cleanup. The Gaze runtime resolves from crates.io (`gaze-pii`, `gaze-recognizers`, `gaze-mcp-core` at 0.11.3). The legacy `GAZE_REPO_TOKEN` PAT is no longer required. Crates.io publish of `gaze-lens` itself is tracked separately. The historical predecessor crate at `reference/debug-proxy/` was removed from the tree in 2026-07 (recover via `git log --follow -- reference/debug-proxy/`); it was never part of the published package or the active build.
