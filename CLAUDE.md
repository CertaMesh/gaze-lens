...
v0.5.0 shipped 2026-06-21 (see CHANGELOG.md); it supersedes the never-tagged 0.4.1 SSH-log work. The Gaze runtime resolves from crates.io (`gaze-pii`, `gaze-recognizers`, `gaze-mcp-core` at 0.11); the bump adopts upstream fail-closed NER. v0.5.0 adds an opt-in `log_grep` keyword mode + redacted-tail cache, `schema`/`list_tables` discovery metadata, restore-telemetry in `replay`, and a `production = true` profile tier that mandates an NER model. The legacy `GAZE_REPO_TOKEN` PAT is no longer required. Crates.io publish of `gaze-lens` itself is tracked separately. The predecessor crate at `reference/debug-proxy/` is historical reference material, excluded from the published package, and not part of the active build.
