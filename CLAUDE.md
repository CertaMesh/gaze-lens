...
The package version is 0.5.5 as of 2026-08-28 (see `Cargo.toml` and CHANGELOG.md); a tag and GitHub Release are separate publication steps. The repository includes cargo-dist release targets and native CI proof for Linux x86_64 and aarch64 alongside Apple Silicon macOS, including the no-DBus keyring failure check, and the canonical Apache-2.0 license template. The Gaze runtime remains on crates.io (`gaze-pii`, `gaze-recognizers`, `gaze-mcp-core` at 0.11.3); this package-version update does not change dependencies or the locked public surface. The legacy `GAZE_REPO_TOKEN` PAT is no longer required. Crates.io publish of `gaze-lens` itself is tracked separately. The historical predecessor crate at `reference/debug-proxy/` was removed from the tree in 2026-07 (recover via `git log --follow -- reference/debug-proxy/`); it was never part of the published package or the active build.
