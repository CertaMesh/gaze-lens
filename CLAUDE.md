...
v0.5.3 shipped 2026-06-23 (see CHANGELOG.md) — adds a no-SSH `kind = "local_log"` source, served `log_grep` schema descriptions, and two dogfood-found security fixes: the keyword-search presence oracle is closed (keyword needle matched raw against the already-redacted window, so raw PII can't be probed) and `serve` now initializes a stderr tracing subscriber so the production-regex warning actually emits. Org refs are `CertaMesh`; docs follow a four-mode structure with a `docs/` index. Built atop v0.5.2 (re-release of v0.5.1 carrying the macOS-arm64 binary asset) and v0.5.1 (log-redaction-default hardening) / v0.5.0 (Gaze 0.11, fail-closed NER, `log_grep` keyword mode + cache, discovery metadata, replay telemetry, `production` NER-mandate tier), which superseded the never-tagged 0.4.1 SSH-log work. The Gaze runtime resolves from crates.io (`gaze-pii`, `gaze-recognizers`, `gaze-mcp-core` at 0.11); the bump adopts upstream fail-closed NER. v0.5.0 adds an opt-in `log_grep` keyword mode + redacted-tail cache, `schema`/`list_tables` discovery metadata, restore-telemetry in `replay`, and a `production = true` profile tier that mandates an NER model. The legacy `GAZE_REPO_TOKEN` PAT is no longer required. Crates.io publish of `gaze-lens` itself is tracked separately. The historical predecessor crate at `reference/debug-proxy/` was removed from the tree in 2026-07 (recover via `git log --follow -- reference/debug-proxy/`); it was never part of the published package or the active build.
