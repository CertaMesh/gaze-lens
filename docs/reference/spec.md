...
## init NER model provisioning amendment

`gaze-lens init` gains an explicit production-profile provisioning capability ON the existing `init` subcommand: it can mark a profile `production = true`, generate or update that profile's policy file with an `[ner].model_dir` (and a fail-closed `[policy] default_action`), and — only when the operator asks — install the pinned NER model bundle via a new `--fetch-model` flag (with `--production` and `--model-dir <path>` companions). This SPEC amendment authorizes that capability as an extension of the existing `init` subcommand; it adds no MCP tool, no CLI subcommand, no source kind, and no retrieval surface. The locked public surface remains exactly 5 MCP tools and 6 CLI subcommands.

**Model identity is single-sourced from the Gaze family.** The provisioned model is the Gaze-pinned **Kiji DistilBERT** bundle (fp32), not the Davlan mBERT wording carried in older gaze-lens docs — that documentation is stale, and its correction is a separate later PR. The bundle identity, immutable Hugging Face repo/commit, required artifact set, `SHA256SUMS` checksum-file digest, and per-artifact hashes are single-sourced from the Gaze family (`gaze-recognizers`); the install/verify path reuses Gaze's shared verifier/installer rather than forking security-critical checksum or filesystem-validation logic into gaze-lens. Model bytes are public artifact data (not operator PII), so the default install directory is the shared Gaze model directory, overridable with `--model-dir`; unlike snapshots, model weights carry no per-file encryption-at-rest expectation.

**Fetch is always an explicit operator action.** `--fetch-model` means "ensure the pinned bundle is installed and verified now." `serve`, `query`, and `check` never fetch model bytes; network access to install weights is an operator action during `init`, never a runtime surprise. Installs use TLS-only URLs under the pinned immutable commit, download into a private temporary directory, verify the `SHA256SUMS` checksum file before any artifact hash, reject missing/malformed artifacts and unsafe filesystem state (symlinks, foreign ownership, insecure modes), set private permissions where the platform supports them, and atomically rename into place only after verification succeeds. Fetched bytes are treated as data and are never executed.

**Provisioning stays fail-closed.** A deferred or failed fetch is not a provisioning event. If the operator skips the fetch or it fails, `init` may still write profile/policy configuration pointing at the intended `model_dir`, but the terminal status is "not provisioned": `check`, `serve`, and `query` must refuse to proceed until the configured bundle verifies, and an explicit `--fetch-model` failure returns non-zero rather than reporting success. `check` gains a targeted model-status **row** (not a new subcommand) that verifies the configured bundle with the same upstream verifier and prints a remediation message on a missing or invalid bundle, while the existing runtime pipeline-load failure remains the final fail-closed backstop. Because model bytes are public artifact data and this path adds no retrieval surface, the `Session::dispatch_tool` / `gaze_mcp_core::PiiEnvelope` chokepoint is correctly untouched by this amendment.
...
- Historical predecessor crate: `crates/debug-proxy/` was extracted from the Gaze monorepo, kept temporarily as `reference/debug-proxy/` for evaluation, and removed from the tree in 2026-07 (PR #110); recover via `git log --follow -- reference/debug-proxy/`.
